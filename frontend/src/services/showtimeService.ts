...
export const showtimeService = {
  ...
  async getShowtimesByDateRange(
    params: GetShowtimesByDateRangeParams,
  ): Promise<Showtime[]> {
  ...
  async getShowtimesByMovieAndTheater(
    movieId: number,
    theaterId: number,
  ): Promise<Showtime[]> {
    const response = await api.get(
      `/showtimes/movie/${movieId}/theater/${theaterId}`,
    );
    return response.data;
  },

  // Get popular showtimes
  async getPopularShowtimes(
    params: GetShowtimesParams = {},
  ): Promise<PageResponse<Showtime>> {
    const { page = 0, size = 10 } = params;
  ...
  getAvailabilityStatus(showtime: Showtime): {
    status: string;
    color: string;
    label: string;
  } {
    ...
    const occupancyRate =
      ((capacity - showtime.availableSeats) / capacity) * 100;
